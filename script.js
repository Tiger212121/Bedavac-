document.addEventListener('DOMContentLoaded', () => {
    ...
    // YENİLENMİŞ Ürün Detay Modal Elementleri
    ...
    const productsDB = [
        { 
            id: 1, name: 'Akıllı Telefon', price: 35000, img: 'https://store.storeimages.cdn-apple.com/4668/as-images.apple.com/is/iphone-15-pro-finish-select-202309-6-1inch-naturaltitanium?wid=5120&hei=2880&fmt=p-jpg&qlt=80&.v=1692845699492', 
            images: [
    ...
    const openModal = (modalElement) => {
        ...
        modalElement.style.display = modalElement.id === 'productDetailModal' ?
 'block' : (modalElement.classList.contains('enhanced') || modalElement.classList.contains('modal-content-profile') ? 'flex' : 'block');
    };
    const closeModal = () => {
    ...
    const updateFavoriteIcons = () => {
        ...
        document.querySelectorAll('.favorite-btn, #favoriteBtnDetail').forEach(btn => {
            ...
            if (user && user.favorites && user.favorites.includes(productId)) {
                ...
                if(btn.id === 'favoriteBtnDetail') btn.title = "Favorilerden Çıkar";
            } else {
    ...
    const renderProfileModal = () => {
        ...
        overviewContainer.innerHTML = `
            <div class="profile-overview-grid">
                <div class="overview-card"><h4>Toplam Sipariş</h4><p>Bugüne kadar verdiğiniz sipariş sayısı.</p><strong>${totalOrders}</strong></div>
                <div class="overview-card"><h4>Toplam Harcama</h4><p>Sitemizde yaptığınız toplam harcama.</p><strong>${totalSpent.toFixed(2)} ₺</strong></div>
                <div class="overview-card"><h4>BC Puanları</h4><p>Mevcut sadakat puanlarınız.</p><strong>${user.bc || 0} BC</strong></div>
            </div>`;
    ...
    const renderProducts = (productsToRender) => {
        ...
        productGrid.innerHTML = productsToRender.map(product => {
            ...
            const vtoButtonHTML = product.category === 'kozmetik' && product.colors ? `<button class="vto-trigger-btn" data-product-id="${product.id}">✨ Sanal Dene</button>` : '';
    ...
    // YENİLENMİŞ ÜRÜN DETAY MODALI FONKSİYONU
    function openProductDetail(product) {
        ...
        detailDescription.textContent = product.attributes || "Bu ürün için detaylı bir açıklama henüz eklenmemiştir.";
        if (product.onSale) {
            detailPriceNew.textContent = `${product.discountPrice.toFixed(2)} ₺`;
            detailDiscountNew.textContent = `${product.price.toFixed(2)} ₺`;
            detailDiscountNew.style.display = 'inline';
        } else {
            detailPriceNew.textContent = `${product.price.toFixed(2)} ₺`;
        ...
        if (product.stock > 5) {
            ...
            detailStockStatus.textContent = `Son ${product.stock} ürün!`;
            detailStockStatus.className = 'stock-status low-stock';
        } else {
            detailStockStatus.textContent = 'Tükendi';
    ...
    // YENİ Ürün Detay Penceresi Etkileşimleri
    productDetailModal.addEventListener('click', e => {
        // Galeri thumbnail'larına tıklama
